...
void app_main(void)
{
    ...
    err += bmp280_read_part_number(&id);
    if(err == ESP_OK){
    ...
    err += bmp280_write_config_filter(FILTER_4);
    bmp280_ctrl_meas_t ctrl_meas = {
        .osrs_press = OSRS_x4,
        .osrs_tmp = OSRS_x1,
    };
    err += bmp280_write_osrs(ctrl_meas);
    ...
    if (err == BMP280_OK && id == 0x58)
    {
        ...
        {
            //Reading here
            if(bmp280_read_data(&bmp280_dt) == BMP280_OK)
            {
