...
void app_main(void)
{
    ...
    err = bmp280_spi_reset();
    if(err != BMP280_OK) ESP_LOGE(TAG, "Error setting the device!");

    err += bmp280_spi_read_part_number(&id);
    if(err == ESP_OK){
    ...
    err += bmp280_spi_set_calib();
    ESP_LOGI(TAG, "Calibration data setting: %s", err == BMP280_OK ? "Successful" : "Failed");

    err += bmp280_spi_write_power_mode(POWERMODE_NORMAL);
    ...
    err += bmp280_spi_write_config_filter(FILTER_4);
    bmp280_ctrl_meas_t ctrl_meas = {
        .osrs_press = OSRS_x4,
        .osrs_tmp = OSRS_x1,
    };
    err += bmp280_spi_write_osrs(ctrl_meas);
    ...
    if (err == BMP280_OK && id == 0x58)
    {
        ...
        {
            //Reading here
            if(bmp280_spi_read_data(&bmp280_dt) == BMP280_OK)
            {
